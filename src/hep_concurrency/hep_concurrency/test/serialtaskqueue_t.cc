...
void
SerialTaskQueue_test::testPush()
{
  std::atomic<unsigned int> count{0};

  hep::concurrency::SerialTaskQueue queue;
  {
    std::shared_ptr<tbb::task> waitTask{
...
void
SerialTaskQueue_test::testPushAndWait()
{
  std::atomic<unsigned int> count{0};
...
void
SerialTaskQueue_test::testPause()
{
  std::atomic<unsigned int> count{0};
...
void
SerialTaskQueue_test::stressTest()
{
  hep::concurrency::SerialTaskQueue queue;
  ...
  while (0 != --index) {
    ...
    std::atomic<bool> waitToStart{true};
    {
      std::thread pushThread([&queue, &waitToStart, pWaitTask, &count] {
        // gcc 4.7 doesn't preserve the 'atomic' nature of waitToStart in the
        // loop
