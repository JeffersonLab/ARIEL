...
namespace mf::service {
  ...
  ELdestination::Category::Config::~Config() = default;
...
to 600.  A value of zero or less disables the timespan functionality.)";
  ...
  ELdestination::Category::~Category() = default;
  ELdestination::Category::Category() = default;

  ELdestination::CategoryParams::~CategoryParams() = default;
  ELdestination::CategoryParams::CategoryParams()
    : limit_{}, reportEvery_{}, timespan_{}
  {}

  ELdestination::XidLimiter::~XidLimiter() = default;
  ...
  ELdestination::MsgFormatSettings::Config::~Config() = default;
  ...
  ELdestination::MsgFormatSettings::~MsgFormatSettings() = default;
  ...
  ELdestination::MsgFormatSettings::timestamp(timeval const& t)
  {
    ...
    char tmpts[SIZE];
    strftime(
      tmpts, sizeof(tmpts), timeFmt_.data(), localtime_r(&t.tv_sec, &timebuf));
    snprintf(ts, sizeof(ts), tmpts, static_cast<unsigned>(t.tv_usec / 1000));
    ret = ts;
    return ret;
  }

  ELdestination::StatsCount::~StatsCount() = default;
  ...
  ELdestination::MsgStatistics::Config::~Config() = default;
  ...
  ELdestination::Config::~Config() = default;
...
===================
    ...
    , charsOnLine_{}
  {
    ...
    for (auto const& category : categories) {
      ...
      categoryParams_[category].reportEvery_ = category_params().reportEvery();
  ...
  ELdestination::formSummary()
  {
    ...
    for (auto const& [xid, count] : statsMap_) {
  ...
  ELdestination::skipMsg(ELextendedID const& xid)
  {
    ...
    if (xl_iter == xidLimiters_.end()) {
      ...
      limiter.skippedMsgCount_ =
        reportEvery - 1; // So that the FIRST of the prescaled messages emerges
