...
void
print_all_fc_metadata_entries_hr(
  vector<FileCatalogMetadataEntry> const& entries,
  ostream& output,
  ostream& /*errors*/)
{
...
void
print_all_fc_metadata_entries_JSON(
  vector<FileCatalogMetadataEntry> const& entries,
  ostream& output,
  ostream& /*errors*/)
{
  ...
  ostream& errors)
{
  ...
  while ((sqlite_status = sqlite3_step(stmt)) == SQLITE_ROW) {
    ...
    ent.name =
      std::string{reinterpret_cast<char const*>(sqlite3_column_text(stmt, 1))};
    ent.value =
      std::string{reinterpret_cast<char const*>(sqlite3_column_text(stmt, 2))};
    all_metadata_entries.push_back(ent);
  }
  if (sqlite_status != SQLITE_DONE) {
    errors << "Unexpected status from table read: " << sqlite3_errmsg(sqliteDB)
           << " (0x" << sqlite_status << ").\n";
  }
                            ...
                            bool want_json)
{
  ...
  if (want_json) {
    ...
    print_all_fc_metadata_entries_JSON(all_metadata_entries, output, errors);
  } else { // Human-readable.
    output << "\nFile catalog metadata from file " << file.GetName() << ":\n\n";
    print_all_fc_metadata_entries_hr(all_metadata_entries, output, errors);
    output << "-------------------------------\n";
  }
  return 0;
}
...
int
print_fc_metadata_from_files(stringvec const& file_names,
                             ostream& output,
                             ostream& errors,
                             bool const want_json)
{
  ...
  for (auto const& fn : file_names) {
    ...
    if (!current_file || current_file->IsZombie()) {
      ++rc;
      errors << "Unable to open file '" << fn << "' for reading."
             << "\nSkipping file.\n";
      continue;
    }
...
void
RootErrorHandler(int level, bool die, char const* location, char const* message)
{
...
main(int argc, char** argv)
{
  ...
  bool const want_json =
    (!vm.count("hr")) && (!vm.count("human-readable")); // Default is JSON.
  ...
  if (file_count < 1) {
    cerr << "One or more input files must be specified;"
         ...
         << "For usage and options list, please do 'sam_metadata_dumper "
            "--help'.\n";
    return 3;
  }
  ...
  // Do the work.
  return print_fc_metadata_from_files(file_names, cout, cerr, want_json);
}
