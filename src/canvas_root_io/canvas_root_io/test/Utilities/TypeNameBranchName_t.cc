...
namespace {
  ...
  class TypeNames {
  public:
    TypeNames() : v_(), valid_(false) {}
    TypeNames(std::vector<std::string>&& v)
      : v_(std::move(v)), valid_((fixV(), v_.size() == expected_size))
    {
      throwIfInvalid_();
    }
    ...
    mangled() const
    {
      return throwIfInvalid_(), v_[0];
    }
    std::string const&
    abi() const
    {
      return throwIfInvalid_(), v_[1];
    }
    std::string const&
    cint() const
    {
      return throwIfInvalid_(), v_[2];
    }
    std::string const&
    reflex() const
    {
      return throwIfInvalid_(), v_[3];
    }
    std::string const&
    branchNameType() const
    {
      return throwIfInvalid_(), v_[4];
    }
    std::string const&
    friendly() const
    {
      return throwIfInvalid_(), v_[5];
    }
  ...
  private:
    ...
    throwIfInvalid_() const
    {
      if (!isValid()) {
        ...
        std::copy(
          v_.begin(), v_.end(), std::ostream_iterator<std::string>(os, ", "));
        if (os.tellp() > 0) {
          os.seekp(-2, std::ios_base::end); // Rewind.
        }
        throw art::Exception(art::errors::DataCorruption)
          << "Attempt to construct or use a TypeNames object with insufficient "
             "input items.\n"
          << "Contents: " << os.str() << "\n";
      }
    }
  ...
  bool
  goodMatchOrThrow(std::smatch const& match, std::string const& line)
  {
    if (match.size() != 2) {
      throw art::Exception(art::errors::LogicError)
        << "INTERNAL ERROR: nonsensical regex match for filename on input "
           "line:\n"
        << line << "\n";
    }
    return true;
  ...
  findDataFileName(std::istream& is)
  {
    ...
    static std::string const col_headers =
      friendlyNameIsBranchNameType ? "Mangled name : ABI-demangled name : Cint "
                                     "name : Reflex name : friendlyName" :
                                     "Mangled name : ABI-demangled name : Cint "
                                     "name : Reflex name : branchNameType : "
                                     "friendlyName";
    while (is) {
      std::smatch fnMatch;
      std::string line;
      ...
      if (std::regex_match(line, fnMatch, fnReg) &&
          ...
          (dataFN = fnMatch[1], std::getline(is, line), line == col_headers) &&
  ...
  bool
  validate(std::ostream& error, TypeNames tn)
  {
    bool result = false;
    ...
    cet::replace_all(
      uniform_type_name, "std::string>", "std::basic_string<char> >");
    cet::replace_all(
      uniform_type_name, "std::string", "std::basic_string<char>");
    ...
    if (uniform_type_name != tn.reflex()) {
      ...
      std::string friendlyName =
        art::friendlyname::friendlyName(uniform_type_name);
      ...
      if (friendlyNameIsBranchNameType) { // Older format, less info.
        ...
        if (compare == "RunAuxiliary" || compare == "SubRunAuxiliary" ||
      ...
      if (friendlyName != compare) {
        error << "ERROR: calculating friendlyName from \"" << uniform_type_name
              << "\":"
              << "       calculated \"" << friendlyName << "\"; should be \""
              << tn.friendly() << "\".\n";
      } else {
        result = true;
      }
...
main(int argc, char** argv)
{
  ...
  while (++argv, --argc) {
    ...
    if (ts) {
      ...
      error << "FATAL ERROR: unable to open file " << fn << ".\n";
      return 1;
    }
    std::string prog, ver;
    ts >> prog >> ver;
    ...
    std::smatch vm;
    if ((prog != "art_typetester") || (!std::regex_match(ver, vm, verRE)) ||
        (vm.size() != 5) || vm[1] != "0" || vm[2] != "00") {
  ...
  out << "INFO: successfully validated " << total_successes << " type"
      << ((total_successes == 1ul) ? "" : "s") << " in " << data_files
      << " data file" << ((data_files == 1ul) ? "" : "s") << " described over "
      << text_files << " text file" << ((text_files == 1ul) ? "" : "s") << "."
      << std::endl;
  return 0;
} // main.
