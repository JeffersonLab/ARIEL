...
{
  if (throw_on_construction_)
    throw_exception_from("ToySource");
...
{
  if (throw_on_closeCurrentFile_)
    throw_exception_from("closeCurrentFile");
  fileData_.clear();
  current_ = iter{};
  end_ = iter{};
}
                             ...
                             art::EventPrincipal*& outE)
{
  ...
  if (throw_on_readNext_)
    throw_exception_from("readNext");
  // Have we any more to read?
  if (current_ == end_)
    return false;

  if (data_.get<bool>("returnTrueWithoutNewFault", false))
    return true;
  ...
  if (eN != -1) {
    ...
    put_product_in_principal(make_unique<bool>(true), *outE, "m2", "b");
    {
      ...
      put_product_in_principal(
        make_unique<decltype(nums)>(std::move(nums)), *outE, "m3");
    }
    put_product_in_principal(
      make_unique<art::Ptr<double>>(sHelper_.makePtr<double>(vtl_, *outE, 1)),
      *outE,
      "m3");
