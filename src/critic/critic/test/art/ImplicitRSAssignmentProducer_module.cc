...
namespace {
  ...
  bool
  insert_failure(art::Exception const& e)
  {
    return e.categoryCode() == art::errors::ProductPutFailure;
  }

  bool
  run_fragment_put(std::string const what)
  {
    return what.find(
  ...
  bool
  run_fragment_full_run_put(std::string const what)
  {
    return what.find(
             "  Cannot put a product corresponding to a full Run using\n"
  ...
  bool
  subRun_fragment_put(std::string const what)
  {
    return what.find(
  ...
  bool
  subRun_fragment_full_subRun_put(std::string const what)
  {
    return what.find(
             "  Cannot put a product corresponding to a full SubRun using\n"
  ...
  public:
    ...
    beginRun(art::Run& r) override
    {
      ...
      BOOST_CHECK_EXCEPTION(
        r.put(make_unique<unsigned>(4), "bgnRunGarbage1", art::runFragment()),
        art::Exception,
    ...
    beginSubRun(art::SubRun& sr) override
    {
      ...
      BOOST_CHECK_EXCEPTION(
        sr.put(
          make_unique<unsigned>(8), "bgnSubRunGarbage1", art::subRunFragment()),
        art::Exception,
        [](art::Exception const& e) {
