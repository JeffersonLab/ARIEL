...
namespace art {
  ...
  namespace {
    ...
    select(BranchType const bt, std::vector<std::string const*> const& strs)
    {
      return (static_cast<size_t>(bt) < strs.size()) ? *strs[bt] :
                                                       undefinedType;
    }
  ...
  std::string const&
  BranchTypeToString(BranchType const bt)
  {
    return select(bt, {&event, &subRun, &run, &results});
  }

  std::string const&
  BranchTypeToProductTreeName(BranchType const bt)
  {
    return select(bt, {&events, &subRuns, &runs, &resultsTree});
  }

  std::string const&
  BranchTypeToMetaDataTreeName(BranchType const bt)
  {
    return select(bt, {&eventMeta, &subRunMeta, &runMeta, &resultsMeta});
  }

  std::string const&
  BranchTypeToAuxiliaryBranchName(BranchType const bt)
  {
    return select(
      bt,
  ...
  std::string const&
  productProvenanceBranchName(BranchType const bt)
  {
    return select(bt,
  ...
  std::string const&
  BranchTypeToMajorIndexName(BranchType const bt)
  {
    return select(bt, {&eventMajorIndex, &subRunMajorIndex, &runMajorIndex});
  }

  std::string const&
  BranchTypeToMinorIndexName(BranchType const bt)
  {
