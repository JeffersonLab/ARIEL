...
function one_file() {
  ...
  # Fix includes 
  perl -wapi\~ -f ${thisdir}/fix_headers_art.pl "${F}" >/dev/null 2>&1 && rm -f "${F}~"
}
...
function cmake_file() {
  ...
  (( ${fix_whitespace:-0} )) && ed "$F" < fix-whitespace.ed > /dev/null 2>&1
  # Fix CMakeLists.txt 
  perl -wapi\~ -f ${thisdir}/fix_cmake_art.pl "${F}" >/dev/null 2>&1 && rm -f "${F}~"
}
...
if [[ -n "${file}" ]]; then
  ...
  for F in `find $TOP \( \( -name .svn -o -name .git -o -name CVS \) -prune \) -o \( -name '*.c' -o -name '*.cxx' -o -name '*.cc' -o -name '*.cpp' -o -name '*.C' -o -name '*.h' -o -name '*.hxx' -o -name '*.hh' -o -name '*.hpp' -o -name '*.[it]cc' -o -name '*.H*' \) -print`; do
    one_file "$F"
  done
  for F in `find $TOP \( \( -name .svn -o -name .git -o -name CVS \) -prune \) -o \( -name CMakeLists.txt -o -name '*.cmake' \) -print`; do
    cmake_file "$F"
  done
  echo
fi
