...
void tex::MakeCombinations::produce(art::Event& event){
  ...
  // Fill histograms
  auto fillHistograms = [this](RecoTrk const& trk){
    double const  m     = trk.momentum().mag();
    double const  sigm  = trk.sigmaMass();
    double const  mpull = (sigm > 0 ) ? (m-mphi_)/sigm : -10.;
  ...
  std::vector<CombinationData> combsData = helper::makeCombinationsAlgLambda(fits, mka_, bz_, fillHistograms);

  // Fill data product
  auto combProduct = std::make_unique<CombinationCollection>();
  auto fillProduct = [this,&combProduct,&fitsHandle](CombinationData const& comb){
  ...
  cet::for_all( combsData, fillProduct );
