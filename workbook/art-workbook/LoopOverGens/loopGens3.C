...

{
  ...
  TFile* file = new TFile( "output/loopGens3.root");
  ...
  TCanvas *canvas = new TCanvas("canvas", "Plots from loopGens3.root" );
  ...
  for ( int i=1; i!=n; ++i ){
    ...
    if ( i==3 ) {
      canvas->Update();
      canvas->Print("output/loopGens3_1.png");
      canvas->Clear();
      canvas->Divide(2,2);
    }
  }
  ...
  canvas->Update();
  canvas->Print("output/loopGens3_2.png");
