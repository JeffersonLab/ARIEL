...
{
  auto pastimes = e.getValidHandle<Pastimes>(pastimeLabel_);
  auto people = e.getValidHandle<People>(peopleLabel_);
  ...
  // into the event.
  auto psd = std::make_unique<PastimeSurveyData>();
  std::vector<std::pair<std::size_t, std::size_t>> indexData
  { { 1, 5 }, { 3, 0 }, { 1, 6 }, { 0, 0 }, { 1, 2 }, { 3, 7 },
    { 2, 1 }, { 2, 8 }, { 2, 3 }, { 0, 1 }, { 4, 8 }, { 2, 0 },
    { 4, 7 }, { 4, 2 }, { 4, 4} };
  for (auto const & p : indexData) {
    psd->addSingle( { people, p.first }, { pastimes, p.second }, *dataIter++);
  }
  e.put(std::move(psd));
  //////////////////////////////////////////////////////////////////////
