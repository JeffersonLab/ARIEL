...
tex::PDT::PDT(fhicl::ParameterSet const& pset, art::ActivityRegistry&)
  ...
  , _verbosity(pset.get<int>("verbosity", 0))
{
  ...
  if (_verbosity > 0) {
    std::cout << "PDT: "
              << " PDT file: " << _pdtFile << " verbosity: " << _verbosity
              << std::endl;
  }
...
{
  for (map_type::const_iterator i = _pdt.begin(); i != _pdt.end(); ++i) {
...
tex::PDT::getById(PDGCode::type id) const
{
  ...
  if (i == _pdt.end()) {
    throw cet::exception("PDT") << "Unrecognized pdg id code: " << id << "\n";
  }
  return i->second;
}

std::ostream&
tex::operator<<(std::ostream& ost, const tex::PDT& t)
{
  t.print(ost);
  return ost;
}
