...
tex::HitMaker::HitMaker(fhicl::ParameterSet const& pset)
  : EDProducer{pset}
  , _intersectionTags(inputTagsFromStrings(
      pset.get<std::vector<std::string>>("intersectionTags")))
...
void
tex::HitMaker::produce(art::Event& event)
{
  ...
  art::ProductID trkHitID(event.getProductID<TrkHitCollection>());
  ...
  for (auto const& handle : handles) {
    ...
    for (auto const& intersection : *handle) {
      ...
      ShellConditions const& cond =
        _conditions->shellConditions(intersection.shell());

      // Apply efficiency.
      if (_flat.fire() > cond.efficiency())
        continue;
      ...
      // Record connection betwween the hit and the intersection from which it
      // was made.
      art::Ptr<Intersection> pintersection(handle,
                                           &intersection - &handle->front());
      art::Ptr<TrkHit> phit(
        trkHitID, hits->size() - 1, event.productGetter(trkHitID));
