...
if [ "`basename $0 2>/dev/null`" = "setup_project.sh" ];then
    echo "You should be sourcing this file"; exit
fi

if [ "${ART_WORKBOOK_BASE_RELEASE}" = '' ];then
    echo "ART_WORKBOOK_BASE_RELEASE is not set; "
    echo "You need to setup a base release of the art Workbook before sourcing this file."
    return 21
fi
