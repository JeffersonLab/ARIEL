...
namespace tex {
  ...
  public:
    ...
    enum symMatrix_index_type { icu  =Helix::icu,
                                iphi0=Helix::iphi0,
                                id0  =Helix::id0,
                                ict  =Helix::ict,
                                iz0  =Helix::iz0 };
    ...
    FittedHelixData const& data() const { return fit_;}

    // Forward the simple accessor functions of the underlying helix class
    Helix               const& helix() const { return fit_.helix(); }
    CLHEP::HepSymMatrix const& cov()   const { return fit_.cov();   }
    hits_type           const& hits()  const { return fit_.hits();  }

    double cu()   const { return fit_.helix().cu();   }
    double phi0() const { return fit_.helix().phi0(); }
    double d0()   const { return fit_.helix().d0();   }
    ...
    CLHEP::Hep3Vector  poca() const{
      return fit_.helix().position(0.);
    }

    CLHEP::Hep3Vector momAtPoca() const{
