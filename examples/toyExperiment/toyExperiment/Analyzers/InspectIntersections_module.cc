//
// Inspect the MC truth about the intersections between tracks and detector
// shells.
//
...
void
tex::InspectIntersections::beginJob()
{
  _hNIntersections = _tfs->make<TH1F>(
    "NIntersections", "Number of intersections per event", 100, 0., 100);
  _nt = _tfs->make<TNtuple>(
    "nt", "Intersections of tracks with shells", "evt:trk:x:y:z");
}

void
tex::InspectIntersections::analyze(const art::Event& event)
{
  ...
  for (auto const& tag : _intersectionTags) {
    ...
    for (auto const& hit : *intersections) {
      ...
      if (doPrint) {
        std::cout << "Hit: " << n++ << " " << hit.genTrack().key() << " "
                  << hit.shell() << " " << hit.position() << std::endl;
      }
    }
  }
