...
namespace tex {
  ...
  public:
    ...
    TrackerComponent
    trackerComponent() const
    {
      return _trackerComponent;
    }
  ...
  inline std::ostream&
  operator<<(std::ostream& ost, const Shell& s)
  {
    ost << s.id() << " " << s.trackerComponent() << " " << s.radius() << " "
        << s.halfLength() << " " << s.halfThickness() << " " << s.sigma();
    return ost;
  }
