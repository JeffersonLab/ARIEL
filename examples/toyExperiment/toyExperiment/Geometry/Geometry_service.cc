...
tex::Geometry::preBeginRun(art::Run const & run) {
  ...
  if ( _verbosity > 0 ){
    std::cout << "Geometry::preBeginRun: "  << " " << run.id() << " Geometry file: " << _geometryFile << std::endl;
  }
  ...
  fhicl::ParameterSet worldPSet = pSetFile.pSet().get<fhicl::ParameterSet>("world");
  makeWorldBox( worldPSet );
  if ( _verbosity > 1 ){
    std::cout << "Half lengths of the world box are: "
	      << _worldHalfLengths[0] << " "
	      << _worldHalfLengths[1] << " "
	      << _worldHalfLengths[2]
	      << std::endl;
...
tex::Geometry::insideWorld( CLHEP::Hep3Vector const& p ){
  if ( std::abs( p.x() ) > _worldHalfLengths[0] ) return false;
  if ( std::abs( p.y() ) > _worldHalfLengths[1] ) return false;
  if ( std::abs( p.z() ) > _worldHalfLengths[2] ) return false;
  return true;
}
